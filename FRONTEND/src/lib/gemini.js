...
export async function generateGoalNarrative(apiKey, prompt) {
  ...
  const res = await fetch(url, {
    method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
      contents: [{ parts: [{ text: prompt }] }], generationConfig: { temperature: 0.65, maxOutputTokens: 1024 }, }), });
  if (!res.ok) {
...
5 to 8 short phrases an app would pass to a scheme search (e.g. "nifty 50 index direct growth", "liquid fund", "parag parikh flexi cap"). No markdown fences, no other text.`;
